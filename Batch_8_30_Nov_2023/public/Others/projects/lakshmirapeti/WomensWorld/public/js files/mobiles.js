...
function loadproducts(data) {
  ...
  var main = document.querySelector('#Pmobile');
  main.appendChild(ultag)

}

function mobilesproducts() {
  axios.get('/mobile/products', {}).then((response) => {
    // console.log(response.data)
    mobileslist = response.data;
    for (var i = 0; i < mobileslist.length; i++) {
      loadproducts(mobileslist[i]);
    }
...
let mobilecarousel = document.getElementById("mobilecarousel")
...
mobilecarousel.addEventListener('click', () => {
  if (document.getElementById("login").style.display == "none" || document.getElementById("signup").style.display == "none") {
    display.remove()
    mobilesproducts()
  }
