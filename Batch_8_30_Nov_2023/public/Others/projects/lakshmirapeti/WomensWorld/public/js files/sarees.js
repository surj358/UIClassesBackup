...
function loadproducts(data) {
  ...
  var main = document.querySelector('#Psarees');
  main.appendChild(ultag)
}

// Groceries
function sarees() {
  axios.get('/saree/products', {}).then((response) => {
    console.log(response.data)
    sareeslist = response.data;
    for (var i = 0; i < sareeslist.length; i++) {
      loadproducts(sareeslist[i]);
    }

  })
}
var womenshide = document.getElementById("product")
var display = document.querySelector("#total")
var Sarees = document.getElementById("sarees");
// Groceries
