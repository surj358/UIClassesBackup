...
function loadproducts(data) {
  ...
  var main = document.querySelector('#Pbeauty');
  main.appendChild(ultag)
}

// Groceries
function beauty() {
  axios.get('/beauty/products', {}).then((response) => {
    console.log(response.data)
    beautylist = response.data;
    for (var i = 0; i < beautylist.length; i++) {
      loadproducts(beautylist[i]);
    }
